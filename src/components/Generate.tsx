...
}> = ({ source, selectedUrl, onSelectUrl }) => {
                ...
                return (
                  ...
                  >
                    <div className="flex-1 pr-4">
                      <div
                        className={`text-sm font-medium line-clamp-1 ${isSelected ? "text-white" : "text-white/80"}`}
                      >
                        {article.title}
                      </div>
                      <div
                        className={`text-xs mt-1 line-clamp-1 ${isSelected ? "text-white/80" : "text-white/40"}`}
                      >
                        {article.summary}
                      </div>
                    </div>
...
export function Generate({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    if (wsStatus === "COMPLETED" && !wsResult && !job?.result && step === 3) {
      console.log(
        "[Generate] WebSocket shows COMPLETED but no result yet, refreshing job status...",
      );
      refreshJobStatus();
    }
  ...
  useEffect(() => {
    ...
    if ((isCompleted || isFailed) && step === 3) {
      if (isCompleted && currentResult) {
        console.log(
          "[Generate] Job completed, building post from result:",
          currentResult,
        );
        // Build Post from job result
        const selectedStyleData = styles.find((s) => s.id === selectedStyle);

        // For URL-based generation, use the original content from job result
        const originalContent =
          inputType === "link"
            ? (currentResult.metadata?.original_content as string) || inputValue
            : inputValue;
        ...
        const post: Post = {
          ...
          originalContent:
            originalContent || "这是您输入的内容转换而来的图文排版。",
  ...
  useEffect(() => {
    if (jobId) {
      console.log("[Generate] WebSocket status:", {
        connected: wsConnected,
        status: wsStatus,
        progress: wsProgress,
        stage: wsStage,
  ...
  const handleGenerate = async () => {
    ...
    try {
      console.log(
        "Calling createGeneration with:",
        inputType === "link" ? "url" : inputType,
        inputValue,
        selectedStyle,
      );
                ...
                {inputType === "text" ? (
                  ...
                  <div className="flex-1 flex flex-col min-h-0">
                    ...
                    <div className="mt-6 flex-1 overflow-y-auto no-scrollbar flex flex-col min-h-0">
                      <h3 className="text-sm font-medium text-white/50 mb-4 shrink-0 px-1">
                        推荐内容
                      </h3>
                      {sourcesLoading && (
              ...
              {stylesLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 text-center">
                  ...
                  <p className="text-white/40 text-sm">
                    Make sure backend is running and assets/styles/ directory
                    exists
                  </p>
                </div>
              ) : (
            ...
            >
              {job?.status === "FAILED" || generationError ? (
                <>
                  ...
                  <h2 className="text-xl font-medium mb-3 text-red-400">
                    生成失败
                  </h2>
